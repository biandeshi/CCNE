...
if __name__ == "__main__":
    ...
    all_embeddings = np.vstack((s_embedding, t_embedding))
    tsne = TSNE(n_components=2)
    all_embeddings_tsne = tsne.fit_transform(all_embeddings)

    s_num = s_embedding.shape[0]
    plt.scatter(all_embeddings_tsne[0:s_num, 0], all_embeddings_tsne[0:s_num, 1], label='Source Network')
    plt.scatter(all_embeddings_tsne[s_num:, 0], all_embeddings_tsne[s_num:, 1], label='Target Network')
    plt.xlabel('t - SNE Dimension 1')
    plt.ylabel('t - SNE Dimension 2')
    plt.title('t - SNE Visualization of Embeddings')
    plt.legend()
    plt.savefig('pics/t-SNE.png')
