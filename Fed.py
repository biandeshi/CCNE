    ...
    for i in range(N):
        ...
        for round in range(args.rounds):
            ...
            for epoch in range(args.epochs):
                globel_model.train()
                globel_optimizer = torch.optim.Adam(globel_model.parameters(), lr=args.lr)
                globel_optimizer.zero_grad()
                in_a, in_b, label = sample(train_anchor)
                label = label.view(-1).to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
                inter_loss = cosine_loss(s_embedding[in_a], t_embedding[in_b], label)
                inter_loss.requires_grad = True
                inter_loss.backward()
                globel_optimizer.step()
