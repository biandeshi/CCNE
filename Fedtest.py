...
def local_train(model, x, edge_index, epochs=10, loss_type='intra'):
    model.train()
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    
    for _ in range(epochs):
        optimizer.zero_grad()
        z = model.s_forward(x, edge_index) if isinstance(model, CCNE) else model(x, edge_index)
        
        # 计算intra_loss
        pos_edge_index = edge_index
        neg_edge_index = torch_geometric.utils.negative_sampling(pos_edge_index, z.size(0))
        recon_loss = model.single_recon_loss(z, pos_edge_index, neg_edge_index)
    ...
    # 返回嵌入（保留梯度）
    return model.s_forward(x, edge_index)
...
if __name__ == "__main__":
    ...
    for i in range(N):
        ...
        global_model = CCNE(s_x.shape[1], t_x.shape[1], args.dim).to(device)
        global_optimizer = torch.optim.Adam(global_model.parameters(), lr=args.lr)
        s_model = CCNE(s_x.shape[1], t_x.shape[1], args.dim).to(device)  # 源网络本地模型
        t_model = CCNE(s_x.shape[1], t_x.shape[1], args.dim).to(device)  # 目标网络本地模型

        # 联邦训练主循环
        for round in range(args.rounds):
            # ------------------------------
            # 阶段1: 本地训练（各网络优化intra_loss）
            # ------------------------------
            # 源网络本地训练
            s_embedding = local_train(
                model=s_model, 
                x=s_x, 
                edge_index=s_e, 
                epochs=args.epochs, 
            ...
            # ------------------------------
            # 阶段2: 服务器计算inter_loss并更新全局模型
            # ------------------------------
            # 采样锚点对
            in_a, in_b, anchor_label = sample(train_anchor, g_s, g_t, neg=args.neg)
            anchor_label = anchor_label.view(-1).to(device)

            # global_s_emb = global_model.s_forward(s_x.to(device), s_e.to(device))
            # global_t_emb = global_model.t_forward(t_x.to(device), t_e.to(device))
            
            # 计算inter_loss（基于本地模型生成的嵌入）
