...
def plot_pK(dataset):
    ...
    for i in range(len(train_file)):
        file = f'output/{dataset}/{train_file[i]}_tr=0.2.txt'
    ...
    plt.tight_layout()
    plt.legend()
    plt.savefig(f'pics/{dataset}/pK.png')
...
def plot_ratio_mrr(dataset):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'pics/{dataset}/ratio_mrr.png')
...
def plot_ratio_pk(dataset):
    ...
    for i in range(len(train_file)):
        ...
        axs.plot([x / 10.0 for x in range(1, 10)], pk, marker=makers[i], label=train_file[i])
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'pics/{dataset}/ratio_p10.png')
